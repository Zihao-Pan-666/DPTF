...
from isddg.features.dynamic_feature_store import build_continuous_table_from_observations, load_pt_feature_table
from isddg.training.continuous_dynamic_prior_v2_trainer import (
    train_continuous_dynamic_prior_v2,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train semantic-conditioned continuous dynamic prior v2 "
                    "(weighted regression + role auxiliary + dynamic BPR)."
    )
    ap.add_argument("--config", default="configs/continuous_dynamic_v2.yaml")
    ...
    dyn_cfg = cfg.get("continuous_dynamic_v2", {})
    paths = cfg.get("paths", {})
    result_dir = Path(paths.get("result_dir", "results/dynamic_role"))
    dynamic_dir = Path(paths.get("dynamic_dir", "artifacts/dynamics"))
    role_dir = Path(paths.get("role_dir", "artifacts/roles/roles_v2/multiscale"))
    ...
    source_table_path = Path(
        ...
        or dyn_cfg.get("source_table_path", dynamic_dir / f"source_continuous_dynamic_table_{source}.pt")
    )
    stats_path = Path(
        args.stats_path
        or dyn_cfg.get("stats_path", dynamic_dir / f"continuous_dynamic_stats_{source}.json")
    )
    ckpt_path = Path(
        args.checkpoint_path
        or dyn_cfg.get("checkpoint", f"artifacts/checkpoints/continuous_dynamic_prior_v2_{source}_seed{seed}.pt")
    )
    results_path = Path(
        args.results_path
        or result_dir / "continuous_dynamic_prior_v2_source_val.csv"
    )
    summary_path = Path(
        args.summary_path
        or result_dir / f"continuous_dynamic_prior_v2_{source}_seed{seed}_summary.json"
    )
    pred_path = Path(
        args.pred_source_table_path
        or dynamic_dir / f"pred_source_continuous_dynamic_table_v2_{source}_seed{seed}.pt"
    )

    print("=" * 80)
    print("[ContinuousDynamicPriorV2] semantic -> continuous dynamic prior")
    ...
    if bool(dyn_cfg.get("use_role_auxiliary", True)):
        if not role_table_path.exists():
            raise FileNotFoundError(f"use_role_auxiliary=True but role table not found: {role_table_path}")
        role_table = load_pt_feature_table(role_table_path, num_items=len(item_map))

    source_val_ranking = bool(dyn_cfg.get("source_val_ranking", True)) and (not args.no_source_val_ranking)

    summary = train_continuous_dynamic_prior_v2(
        item_features=sem,
        ...
        checkpoint_extra={
            ...
            "protocol": {
                "name": "semantic_conditioned_continuous_dynamic_prior_v2",
    ...
    model, ckpt = load_continuous_v2_from_checkpoint(ckpt_path, device)
    pred_source_table = predict_continuous_table_v2(model, sem, device=device)
    ...
    # Flatten the best epoch row for quick spreadsheet comparison.
    ...
    append_csv(
        {
            "model": "continuous_dynamic_prior_v2",
            "stage": "source_prior",
            "source": source,
            "seed": seed,
            "checkpoint": str(ckpt_path),
