...
from isddg.evaluation.sequence_prototype_evaluator import evaluate_sequence_prototype, parse_float_grid
...
def build_candidate_dynamic_table(args, cfg, sem: torch.Tensor, num_items: int, device: torch.device) -> tuple[torch.Tensor | None, str]:
    proto_cfg = cfg.get("prototype", {})
    source = cfg.get("source", "amazon_movies_and_tv")
    mode = args.candidate_dynamic_source or proto_cfg.get("candidate_dynamic_source", "predicted")
    if mode in {"none", "off"}:
        return None, "none"
    if mode == "oracle":
        path = Path(args.candidate_dynamic_table or proto_cfg.get("source_dynamic_table", ""))
        if not path.exists():
            raise FileNotFoundError(f"candidate_dynamic_source=oracle but table not found: {path}")
    ...
    if mode == "predicted":
        ...
        if not ckpt.exists():
            print(f"[CandidateDynamic] predicted requested but checkpoint not found: {ckpt}. Use dynamic branch OFF.")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Tune source-validation fusion weights for sequence dynamic prototypes.")
    ...
    ap.add_argument("--candidate_dynamic_source", choices=["predicted", "oracle", "table", "none"], default=None)
    ap.add_argument("--candidate_dynamic_table", default=None)
    ap.add_argument("--beta_sem_grid", default=None)
    ap.add_argument("--beta_dyn_grid", default=None)
    ...
    beta_sem_grid = _as_list(args.beta_sem_grid if args.beta_sem_grid is not None else proto_cfg.get("beta_sem_grid", [0.0]))
    beta_dyn_grid = _as_list(args.beta_dyn_grid if args.beta_dyn_grid is not None else proto_cfg.get("beta_dyn_grid", [0.0]))
    select_metric = str(proto_cfg.get("select_metric", "NDCG@10"))
    ...
    results_path = Path(args.results_path or result_dir / "sequence_prototype_source_val.csv")
    summary_path = Path(args.summary_path or result_dir / f"sequence_prototype_{source}_seed{seed}_summary.json")

    set_seed(seed)
    device = get_device(args.device or cfg_get(cfg, "training", "device", "auto"))

    print("=" * 80)
    print("[SequencePrototype] source-validation tuning")
    print(f"source={source}")
    print(f"semantic_checkpoint={semantic_checkpoint}")
    print(f"prototype_bank={prototype_bank}")
    print(f"beta_sem_grid={beta_sem_grid}")
    print(f"beta_dyn_grid={beta_dyn_grid}")
    ...
    sem = load_semantic_embeddings(data_root, source, item_map=item_map, embedding_dir=embedding_dir, strict=True)[: len(item_map) + 1]
    sem[0] = 0.0
    ...
    candidate_dyn, candidate_dyn_source = build_candidate_dynamic_table(args, cfg, sem, len(item_map), device)

    loader = DataLoader(
        PrefixDataset(val_samples, len(item_map), max_len),
    ...
    rows, best = [], None
    for beta_sem in beta_sem_grid:
        for beta_dyn in beta_dyn_grid:
            metrics = evaluate_sequence_prototype(
                ...
                tie_policy="worst",
                desc=f"proto sem={beta_sem} dyn={beta_dyn}",
                show_progress=show_progress,
                collect_score_stats=(best is None),
            )
            row = {
            ...
            if best is None or float(row[select_metric]) > float(best[select_metric]):
                best = row
            print(
                f"[beta_sem={beta_sem} beta_dyn={beta_dyn}] "
                f"R@10={metrics['Recall@10']:.6f} N@10={metrics['NDCG@10']:.6f} "
    ...
    summary = {
        ...
        "best_beta_sem": float(best["beta_sem"]),
        "best_beta_dyn": float(best["beta_dyn"]),
        "best_metric": float(best[select_metric]),
        "best_row": best,
        "semantic_checkpoint": str(semantic_checkpoint),
        "prototype_bank": str(prototype_bank),
        "candidate_dynamic_source": candidate_dyn_source,
        ...
        "protocol": {
            ...
            "target_usage": "none_during_tuning",
        },
    }
    save_json(summary, summary_path)
    print(f"[Best] beta_sem={summary['best_beta_sem']} beta_dyn={summary['best_beta_dyn']} {select_metric}={summary['best_metric']:.6f}")
