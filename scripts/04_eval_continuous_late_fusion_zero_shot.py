...
def main():
    ap = argparse.ArgumentParser(
        description="Zero-shot evaluation for continuous dynamic late fusion."
    )
    ...
    summary_path = Path(
        args.beta_summary
        or result_dir / f"continuous_late_fusion_predicted_{source}_seed{seed}_summary.json"
    )
    if args.beta is not None:
        ...
        beta = float(obj.get("best_beta", 0.0))
        beta_source = str(summary_path)
    else:
        beta = 0.0
    ...
    for target in targets:
        ...
        loader = DataLoader(
            PrefixDataset(samples, len(item_map), max_len),
        ...
        row = {
            ...
            "continuous_checkpoint": str(cont_ckpt),
            "checkpoint_kind": checkpoint_kind,
            "beta": beta,
            "beta_source": beta_source,
            "ranking_mode": ranking,
            "eval_negatives": eval_negatives,
            "num_target_users": len(samples),
        ...
        out_json = result_dir / f"continuous_late_fusion_{source}_to_{target}_{ranking}_seed{seed}.json"
        save_json(row, out_json)

        print(
            f"[target={target}] R@10={metrics['Recall@10']:.6f} "
