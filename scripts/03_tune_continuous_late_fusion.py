...
def main():
    ap = argparse.ArgumentParser(
        description="Tune source-validation late fusion with continuous dynamic prior."
    )
    ap.add_argument("--config", default="configs/continuous_dynamic.yaml")
    ap.add_argument("--dynamic_source", default="predicted", choices=["predicted", "oracle", "table"])
    ...
    continuous_table_path = Path(
        args.continuous_table_path or cfg_get(cfg, "continuous_dynamic", "source_table_path", "")
    )

    beta_grid = (
        parse_beta_grid(args.beta_grid)
        if args.beta_grid
        else parse_beta_grid(cfg_get(cfg, "late_fusion", "beta_grid", [0.0]))
    )
    ...
    results_path = Path(
        args.results_path
        or result_dir / f"continuous_late_fusion_{args.dynamic_source}_source_val.csv"
    )
    summary_path = Path(
        args.summary_path
        or result_dir / f"continuous_late_fusion_{args.dynamic_source}_{source}_seed{seed}_summary.json"
    )
    ...
    if args.dynamic_source == "oracle":
        ...
        checkpoint_kind = "oracle_table"
    else:
        predictor, _, checkpoint_kind = load_continuous_dynamic_predictor_from_checkpoint(cont_ckpt, device)
        dyn_table = predict_continuous_dynamic_table(predictor, sem, device=device)
    ...
    for beta in beta_grid:
        metrics = evaluate_dynamic_signal(
            model,
            dyn_table,
            loader,
            len(item_map),
            ...
            desc=f"cont-dyn {args.dynamic_source} beta={beta}",
        )
        row = {
            "model": "continuous_late_fusion",
            "stage": "source_val",
            "source": source,
            "target": "",
            "seed": seed,
            ...
            "feature_type": f"continuous_{args.dynamic_source}",
        ...
        print(
            f"[beta={beta}] R@10={metrics['Recall@10']:.6f} "
            f"N@10={metrics['NDCG@10']:.6f} MRR@10={metrics['MRR@10']:.6f}",
            flush=True,
        )
