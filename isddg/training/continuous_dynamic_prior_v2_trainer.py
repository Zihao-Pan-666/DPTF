...
class SemanticConditionedContinuousDynamicPrior(nn.Module):
    ...
    It predicts a low-dimensional continuous dynamic vector from frozen item text
    embeddings. Optionally, it also predicts the soft dynamic-role distribution
    as an auxiliary structured target.

    The model intentionally does NOT use target-domain interactions. At target
    inference time, the same semantic -> dynamic mapper is applied to target item
...
def _gather_predicted_vectors(
    ...
    """
    Efficiently predict dynamic vectors only for the unique item ids appearing in
    the current sequence-ranking batch, then restore the original tensor shape.
    """
...
def _history_state_from_predicted(
    ...
    if pooling == "last":
        idx = (lengths - 1).view(B, 1, 1).expand(B, 1, history_dyn.size(-1))
...
def dynamic_bpr_loss_from_batch(
    ...
    state = _history_state_from_predicted(
        history_dyn=history_dyn,
        history_ids=history,
        pooling=pooling,
        recent_k=recent_k,
        decay=decay,
...
def _make_source_val_loader(
    ...
    if not val_samples:
        return None
    return DataLoader(
        PrefixDataset(val_samples, num_items=num_items, max_len=max_len),
...
@torch.no_grad()
def predict_continuous_table_v2(
    model: nn.Module,
    item_features: torch.Tensor,
    batch_size: int = 4096,
    ...
    for s in range(0, item_features.size(0), batch_size):
        out = model(item_features[s : s + batch_size].to(device))["dynamic"]
...
def train_continuous_dynamic_prior_v2(
    item_features: torch.Tensor,
    ...
    for epoch in range(1, epochs + 1):
        ...
        pbar = tqdm(
            range(0, train_idx.numel(), batch_size),
            desc=f"cont-dyn-prior-v2 epoch {epoch:03d}",
            leave=False,
        )

        for s in pbar:
            idx = train_idx[s : s + batch_size].to(device)
            out = model(x[idx])
            reg_loss = weighted_regression_loss(
                out["dynamic"],
                y[idx],
            ...
            if lambda_bpr > 0 and seq_iter is not None:
                ...
                neg = _sample_training_negatives_fast(
                    hist,
                    pos,
                    int(num_items or x_cpu.size(0) - 1),
                    int(bpr_negatives),
                    device,
                )
                bpr_loss = dynamic_bpr_loss_from_batch(
                    model=model,
                    item_features=x,
                    history=hist,
                    pos_items=pos,
            ...
            pbar.set_postfix(loss=f"{total_loss / max(total_n, 1):.6f}")

        model.eval()
        val_preds = []
        with torch.no_grad():
            for s in range(0, val_idx.numel(), batch_size):
                idx = val_idx[s : s + batch_size].to(device)
        ...
        row["val_total_loss"] = (
            float(lambda_reg) * row["val_reg_mse"]
            + (float(lambda_role) * float(val_role_loss) if val_role_loss is not None else 0.0)
        )

        if val_loader is not None:
            pred_table = predict_continuous_table_v2(model, x_cpu, batch_size=4096, device=device)
        ...
        msg = (
            f"[ContDynPriorV2][epoch={epoch:03d}] "
...
def load_continuous_v2_from_checkpoint(path: str | Path, device: torch.device):
