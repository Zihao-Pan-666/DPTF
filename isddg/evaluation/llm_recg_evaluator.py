...
def evaluate_llm_recg_ranking(
    ...
    progress_desc: str = "eval bert4rec_recg",
) -> Dict[str, float]:
    ...
    for batch in tqdm(loader, desc=progress_desc, leave=False, disable=not show_progress):
        hist = batch["history"].to(device, non_blocking=True)
        pos = batch["target"].to(device, non_blocking=True)
        batch_size = int(hist.size(0))
        ...
        if ranking_mode == "sampled":
            ...
            scores = model.score(hist, candidates, is_target_domain=is_target_domain).detach().cpu().numpy()
            ...
            scores = model.score_all(hist, is_target_domain=is_target_domain)
            scores[:, 0] = -torch.inf
